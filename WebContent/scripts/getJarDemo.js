...
//then lookup the corresponding image src from the NextImgArray. could have been improved by using a single json object ?

$(document).ready(function() {
   $('.galleryRhtNav ')
   		.click(function() {
   			var currentImgSel=$('.floatLeft.indImgWrapper > img');
   			var currentImgSrc=currentImgSel[1].src;
   			...
   			if (nextIdx < NS.catgNextImgArray.length) {
	   			...
	   			currentImgSel[0].src=currentImgSrc;
	   			currentImgSel[1].src=prefix+'/'+nextImg;
   			}	
   		});
   $('.galleryLftNav ')
		.click(function() {
   			var currentImgSel=$('.floatLeft.indImgWrapper > img');
			var currentImgSrc=currentImgSel[0].src;
			var nextIdx=NS.catgImgArray[currentImgSrc.substring(currentImgSrc.lastIndexOf('/')+1)]-1;
   			var nextImg,prefix;
			if (nextIdx >= 0) {
	   			prefix=currentImgSrc.substr(0,currentImgSrc.lastIndexOf('/'));
	  			nextImg=NS.catgNextImgArray[nextIdx];
	  			currentImgSel[0].src=prefix+'/'+nextImg;
	  			currentImgSel[1].src=currentImgSrc;
			}	
		});
 });
